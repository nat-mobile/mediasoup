...
class Server extends EventEmitter
{
	constructor(options)
	{
        ...
        if (!check.lessOrEqual(options.vp9MinSpartial, 1))
			options.vp9MinSpartial = 1;
        if (!check.greaterOrEqual(options.vp9MinSpartial, 0))
			options.vp9MinSpartial = 0;
        
        if (!check.lessOrEqual(options.vp9MinTemporial, 2))
			options.vp9MinTemporial = 2;
        if (!check.greaterOrEqual(options.vp9MinTemporial, 0))
			options.vp9MinTemporial = 0;
